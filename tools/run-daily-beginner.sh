#!/bin/bash
# DCLビギナーランキング 日次更新（スケジュールタスク tiktok-beginner-ranking-daily から呼ぶ）
...
#   診断ログは tools/daily-beginner.log と標準エラーへ。標準出力はSlack本文のみ。
set -uo pipefail
...
if [ -z "$MSG" ]; then
  say "❌ 生成失敗（TSVなし等）"
  echo "<@U0A6WU3P3LL>"
  echo "⚠️ DCLビギナーランキング 自動更新に失敗（データTSVが見つからない等）。サイトは更新していません。"
  exit 1
fi
...
if ! python3 build.py >>"$LOG" 2>&1; then
  say "❌ build失敗"
  echo "<@U0A6WU3P3LL>"
  echo "⚠️ DCLビギナーランキング build失敗。サイトは更新していません。"
  exit 1
fi

# 3. 変更があれば push
if ! git diff --quiet || ! git diff --cached --quiet; then
  if git add -A && git commit -q -m "daily: ビギナーランキング更新 ($DATE)" && git push -q >>"$LOG" 2>&1; then
    say "push完了"
  else
    say "❌ push失敗"
    echo "<@U0A6WU3P3LL>"
    echo "⚠️ DCLビギナーランキング git push失敗。ローカルは更新済みですが公開反映していません。"
    exit 1
  fi
else
  say "変更なし（pushスキップ）"
